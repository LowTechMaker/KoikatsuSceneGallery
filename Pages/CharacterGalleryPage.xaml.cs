...
public sealed partial class CharacterGalleryPage : Page
{
    ...
    // Same cell-fitting engine as the scene gallery, but character cards are
    // portrait (252x352), so the image ratio is height/width = 352/252. See
    // GalleryPage for the full rationale behind the column-fit approach.
    ...
    // The three fixed sizes: small → medium → large (card width px). Shared with
    // the scene gallery's persisted ThumbnailWidth setting.
    ...
    {
        ViewModel = App.CharacterGalleryViewModel;
        InitializeComponent();
