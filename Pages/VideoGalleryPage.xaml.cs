...
public sealed partial class VideoGalleryPage : Page
{
    ...
    {
        ViewModel = App.VideoGalleryViewModel;
        InitializeComponent();
        ...
        App.SettingsViewModel.VideoFolderPathsChanged += OnFolderPathsChanged;
    }
    ...
    {
        var paths = e.Items.OfType<MediaCard>().Select(c => c.FilePath).ToList();
    ...
    private void GalleryGrid_ItemClick(object sender, ItemClickEventArgs e)
    {
        if (e.ClickedItem is MediaCard card)
            Frame.Navigate(typeof(VideoDetailPage), card);
    }

    private void RandomButton_Click(object sender, RoutedEventArgs e)
    {
        var card = ViewModel.GetRandomCard();
        if (card != null) Frame.Navigate(typeof(VideoDetailPage), card);
    }
